<template>
  <div class="flex items-center justify-center space-x-1.5"
    :class="isLoading ? 'cursor-wait' : 'cursor-pointer'" @click.stop="handleLike">
    <UIcon v-if="isLoading" name="i-hugeicons:refresh" class="size-5 text-primary animate-spin" />

    <UIcon v-else :name="liked ? 'i-hugeicons:heart-check' : 'i-hugeicons:favourite'" :class="[
...
const handleLike = async () => {
  ...
  try {
    ...
    setTimeout(() => { isManualClick.value = false; }, 1000);
  }
};
