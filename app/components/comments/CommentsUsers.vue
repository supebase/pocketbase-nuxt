<template>
  <div ref="target" class="flex mt-2.5 h-6">
    <div
      v-if="status === 'pending' && usersToShow.length === 0"
      ...
      <UIcon
        :name="
          !allowComment
            ...
            : 'i-hugeicons:comment-02'
        "
        class="size-4.5"
      />
      <span class="text-sm">{{
        !allowComment ? '评论已关闭' : '暂无评论'
      }}</span>
    </div>
  </div>
</template>
...
const totalCount = computed(
    () => commentsResponse.value?.data?.totalItems || 0,
);

const remainingCount = computed(() =>
    Math.max(0, totalCount.value - usersToShow.value.length),
);
</script>
