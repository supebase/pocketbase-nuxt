    ...
    <div v-else-if="comments.length > 0">
      ...
      >
          ...
          <div class="flex items-center justify-between text-base font-medium">
            ...
            <div class="flex items-center justify-center gap-5">
              ...
              <CommonLikeButton
                ...
                @like-change="
                  (liked, lks) => handleLikeChange(liked, lks, item.id, false)
                "
              />
            </div>
          </div>
        ...
        <template #description="{ item }">
          <div
            class="text-base tracking-wide leading-6 hyphens-none whitespace-pre-wrap"
          >
            <template
              v-for="(part, index) in parsedContent(item.comment)"
              :key="index"
            >
              <span v-if="part.isMention" class="text-primary font-semibold">{{
                part.text
              }}</span>
...
watch([loading, isLoadingMore], ([l1, l2]) =>
    emit('loading-change', l1 || l2),
);
