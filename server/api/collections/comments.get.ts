...
export default defineApiHandler(async (event): Promise<CommentsListResponse> => {
	...
	const { page, perPage, query } = getPaginationParams(event, { page: 1, perPage: 20, maxPerPage: 100 });
	...
	// 步骤 3: 调用服务
	const { items, totalItems, page: currentPage, perPage: currentPerPage } = await getCommentsList(pb, page, perPage, filter, userId);
