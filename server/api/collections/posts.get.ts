...
export default defineApiHandler(async (event): Promise<PostsListResponse> => {
	...
	// 步骤 1: 使用工具函数提取分页参数
	const { page, perPage, query } = getPaginationParams(event, { page: 1, perPage: 10, maxPerPage: 100 });

	// 步骤 2: 提取业务搜索参数
	const keyword = query.q as string | undefined;

	// 步骤 3: 调用服务
	const { items, totalItems, page: currentPage, perPage: currentPerPage } = await getPostsList(pb, page, perPage, keyword);
